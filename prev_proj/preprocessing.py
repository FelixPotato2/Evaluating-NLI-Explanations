    ...
    def extract_highlighted_words(self):
        ...
        for i in range(1, 4):  # Iterate through all 3 explanations
            self.data[f'Sentence1_Highlighted_Words_{i}'] = self.data.apply(
                lambda row: self._parse_highlighted_words(row[f'Sentence1_Highlighted_{i}'], row['Sentence1']), axis=1 # Ester highlighted are indeces and gives those to extract words at those indeces
